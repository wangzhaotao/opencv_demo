...
{
    //赋值给绘图视图
    [self.pointView addARectWithPoints:_originalCornerPoints];
}
//边角点扩大至图片视图大小
-(void)resetBiggestRectDetectPoints
{
    ...
    NSArray *points = @[@[[NSNumber numberWithFloat:0],[NSNumber numberWithFloat:0]],
    ...
    for (int c = 0; c < 1; c++) { // 3-->1 wzt20170405
        ...
        if (c==0)
        {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                //赋值给绘图视图
                [self.pointView clearBeforeCircleAndLines];
                [self.pointView addARectWithPoints:temp];
            });
        }
